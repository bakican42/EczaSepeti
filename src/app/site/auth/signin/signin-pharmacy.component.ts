...
@Component({
  selector: 'app-signin-pharmacy',
  templateUrl: './signin-pharmacy.component.html',
  styleUrls: []
})
export class SigninPharmacyComponent implements OnInit {
  ...
  authFormSubmit() {
    if (this.authFormGroup.valid) {
      const pharmacyDataLogin = {
        email: this.authFormGroup.value.email,
        password: this.authFormGroup.value.password
      }
      // login
      this.authService.apiPharmacyLogin(pharmacyDataLogin).subscribe(response => {
        if (response.success) {
